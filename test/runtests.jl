...
@testset "Most-Recent-Cohorts-Scorecard-Elements.csv.gz" begin
    ...
    @test_warn "Large values for `typedetectrows` will reduce performance. Consider manually declaring the types of columns using the `types` argument instead." uCSV.read(GDS(open(f)), header=1, encodings=Dict("NULL" => missing, "PrivacySuppressed" => missing), typedetectrows=7283, quotes='"', skiprows=2:typemax(Int))
